/**
 * @file snake_control_request.cpp
...
ros::Publisher SnakeControlRequest::pub_imu_command_;
ros::Publisher SnakeControlRequest::pub_cop_command_;

/** @fn
 * @brief サーボのIDのリストをSlaveMCUに書き込む
 */
void SnakeControlRequest::RequestJointSetIDList() {
  snake_msgs::SnakeJointCommand joint_command;
  joint_command.set_id_list = true;
  pub_joint_command_.publish(joint_command);
}

/** @fn
 * @brief サーボのIDのリストをSlaveMCUから読み込む
 */
void SnakeControlRequest::RequestJointReadIDList() {
  snake_msgs::SnakeJointCommand joint_command;
  joint_command.read_id_list = true;
  pub_joint_command_.publish(joint_command);
}
 ...
 * @brief 全ての関節にPingをうつ
 */
void SnakeControlRequest::RequestJointPingAll() {
  snake_msgs::SnakeJointCommand joint_command;
...
void SnakeControlRequest::RequestJointSetPositionRange(std::vector<double> joint_angle, int32_t start, int32_t last) {
  snake_msgs::SnakeJointCommand joint_command;
  snake_msgs::SnakeJointData joint_taeget_position;
 ...
 * @param uint8_t joint_index 関節番号
 */
void SnakeControlRequest::RequestJointSetPIDGain(uint8_t joint_index, uint16_t p_gain, uint16_t i_gain, uint16_t d_gain) {
  snake_msgs::SnakeJointCommand joint_command;
  joint_command.joint_index = joint_index;
  joint_command.set_pid_gain = true;
  joint_command.p_gain = p_gain;
...
//=== IMU ===============================================//
/** @fn
 * @brief IMUの情報をRollPitchYawの形式で読み出す
 * @param なし
 * @return なし
 * @detail
 */
void SnakeControlRequest::RequestIMURollPitchYaw(uint8_t imu_index) {
  snake_msgs::SnakeIMUCommand imu_command;
  imu_command.imu_index = imu_index;
  imu_command.read_roll_pitch_yaw = true;
  pub_imu_command_.publish(imu_command);
}

/** @fn
 * @brief IMUから角速度情報を読み出す
 * @param なし
 * @return なし
 * @detail
 */
void SnakeControlRequest::RequestIMUGyro(uint8_t imu_index) {
  snake_msgs::SnakeIMUCommand imu_command;
  imu_command.imu_index = imu_index;
  imu_command.read_gyro = true;
  pub_imu_command_.publish(imu_command);
}
...
//=== COP ===============================================//
/** @fn
 * @brief COPの情報を形式で読み出す
 * @param なし
 * @return なし
 * @detail
 */
void SnakeControlRequest::RequestCOPData(uint8_t cop_index) {
  snake_msgs::SnakeCOPCommand cop_command;
  cop_command.cop_index = cop_index;
  cop_command.target_all = false;
  cop_command.read_data = true;
  pub_cop_command_.publish(cop_command);
}

void SnakeControlRequest::RequestCOPDataAll() {
  snake_msgs::SnakeCOPCommand cop_command;
  cop_command.cop_index = 0;
  cop_command.target_all = true;
  cop_command.read_data = true;
  pub_cop_command_.publish(cop_command);
}
